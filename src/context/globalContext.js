...
export const GlobalProvider = ({ children }) => {
    ...
    const spendingPerBudget = useMemo(() => {
        return activeBudgets.map(budget => {
            ...
            const totalSpent = filteredExpenses.reduce((acc, curr) => acc + parseFloat(curr.amount || 0), 0);
            console.log("Total Spent for", budget.name, ":", totalSpent);
    
            return {
                ...budget,
                spent: totalSpent,
                status: totalSpent < budget.amount ? 'Under Budget' : totalSpent === budget.amount ? 'On Budget' : 'Over Budget',
            };
        });
