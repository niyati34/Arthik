...
// Removed duplicate Navigation function declaration

// Enhanced Navigation for scalability (Oct 16, 2024)
// Removed duplicate React and Link import
        ...
        {navLinks.map((link, idx) => (
          <li key={link.name} className={active === idx + 1 ? "active" : ""}>
            <Link
              ...
              aria-current={active === idx + 1 ? "page" : undefined}
            >
              {link.name}
            </Link>
          </li>
...
const glowAnimation = keyframes`
    0% {
        box-shadow: 0 0 8px #00ffab;
    }
    100% {
        box-shadow: 0 0 24px #00ffab;
    }
`;
...
const MenuItem = styled.li`
  ...
  .item-box {
    ...
    span {
      ...
      text-shadow: ${(props) =>
        props.isActive ? "0px 0px 12px #00ffab" : "none"};
    }
  }
  ${(props) =>
    props.isActive &&
